...
html"""
<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/19/Benoit_Mandelbrot_mg_1804.jpg/220px-Benoit_Mandelbrot_mg_1804.jpg"
...
md"""
Die Mandelbrot-Menge ist definiert durch die rekursive Abbildung
...
#==
Methode zur Berechnung der Mandelbrot-Iteration
...
function mandelbrot_set(Z::Matrix{T},max_iter=255)::Matrix{Int} where T <:Number
	...
	for row_index = 1:n_r
		for column_index = 1:n_c
			result[row_index,column_index]=mandelbrot_map(Z[row_index,column_index],max_iter)
		end
	end
	result
end
...
md"""
Für Julia-Mengen verweise ich auf den [Wikipedia-Artikel](https://de.wikipedia.org/wiki/Julia-Menge).
"""

# ╔═╡ b912331d-e1c4-495a-90f9-5553db7e2d83
#==
"""
Methode zur Berechnung der Julia-Iteration für das Polynom

f(z)=z^2+c

z: Startpunkt der Iteration für die Julia-Menge
...
function julia_set(Z::Matrix{T},c::T,max_iter::Int)::Matrix{Int} where T <:Number
	...
	for row_index = 1:n_r
		for column_index = 1:n_c
			result[row_index,column_index]=julia_map(Z[row_index,column_index],c,max_iter)
		end
	end
	result
end
...
md"""
Julia-Menge für Parameter $c=0+i*0 \in \mathbb{C}$ ist ein Kreis.
"""
...
1. Führe über [PlutoUI](https://docs.juliahub.com/PlutoUI/) Felder für die Komponenten $a,\; b $ der komplexen Zahl $c=a+i \;b$ ein, so dass man definierte Werte eingeben kann. Beispiele für PlutoUI findet man [hier](https://featured.plutojl.org/basic/plutoui.jl). Versuche Beispiel von [hier](https://de.wikipedia.org/wiki/Julia-Menge#/media/Datei:Julia-Teppich.png) zu reproduzieren.
2. Was passiert, wenn man in der Funktion `julia_map` das Polynom $z^2+c$ durch andere Polynome ersetzt? (Achtung: es ist wichtig $c$ zu behalten)
3. Was müssen wir machen, wenn wir das Funktionen auf andere Zahlentypen, z.B. [Anormale-komplexe Zahlen](https://de.wikipedia.org/wiki/Anormal-komplexe_Zahl), erweitern wollen?
"""
...
1. [Mandelbrot-Menge](https://de.wikipedia.org/wiki/Mandelbrot-Menge)
2. [Julia-Menge](https://en.wikipedia.org/wiki/Julia_set)
"""
